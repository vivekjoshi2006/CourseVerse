...
// Updated Categories Configuration with Programming Domain
...
// Mapping for Dynamic Topic Tags
const DEV_TAG_MAP = {
  // Programming
  'Python': 'python',
...
// Accurate Multi-Domain Classifier
...
async function fetchAllLiveStreamsAuto() {
  if (isIngesting) return cachedCourses;
  isIngesting = true;
  ...
  // 2. Loop Through All Categories & Subcategories
  CATEGORIES_CONFIG.forEach(({ category, subcategories }) => {
    subcategories.forEach((subcat) => {
      ...
      // DEV Community & CodeCamp Streams (fetching dynamic articles per topic)
      tasks.push(async () => {
        try {
          ...
          // Next 2 articles -> CodeCamp (Topic Specific Track)
      ...
      // Stanford & Oxford Audio Lecture Stream
      tasks.push(async () => {
        try {
      ...
      // WikiBooks Open Syllabus Stream
      tasks.push(async () => {
        try {
      ...
      // Hacker Hub Engineering Guides Stream
      tasks.push(async () => {
        try {
  ...
  console.log(`Ingestion Complete: Ingested ${shuffled.length} tracks.\n`);
